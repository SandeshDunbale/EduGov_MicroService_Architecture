...
public class ModelMapperConfig {
	...
	public ModelMapper modelMapper() {
		...
		// 1. Map Project details to GrantResponseDTO
		modelMapper.typeMap(Grant.class, GrantResponseDTO.class).addMappings(mapper -> {
			mapper.map(src -> src.getProject().getProjectId(), GrantResponseDTO::setProjectId); 
			mapper.map(src -> src.getProject().getTitle(), GrantResponseDTO::setProjectTitle);
		});

		// 2. Map Project details to GrantApplicationDTO (THIS IS THE ONE THAT WAS TYPO'D!)
