...
//    }
 ...
 // OPTIONAL: Helper for extracting Faculty ID for downstream routing
    public Long extractFacultyId(final String token) {
        Claims claims = getClaims(token);
        return claims.get("facultyId", Long.class);
    }

    // OPTIONAL: Helper for extracting Student ID for downstream routing
    public Long extractStudentId(final String token) {
        Claims claims = getClaims(token);
