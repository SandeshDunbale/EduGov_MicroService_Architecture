		...
		return builder.routes()
				...
				.route("public-auth",
						r -> r.path("/api/auth/**", "/api/users/login", "/api/users/recoverEmail",
								"/api/users/resetPassword", "/api/identity/register").uri("lb://IDENTITYSERVICEEDUGOV")) // Assuming
																															// registration
																															// hits
																															// identity
																															// first
				...
				.route("resource-general-view",
						r -> r.method("GET").and().path("/api/requests/**")
								.filters(f -> f.filter(authFilter.apply(new AuthenticationFilter.Config()))) // No
																												// specific
																												// roles,
																												// just
																												// must
